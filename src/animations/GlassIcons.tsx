...
const gradientMapping: Record<string, string> = {
  ...
  pink: "linear-gradient(hsl(265.86, 31.52%, 63.92%),hsl(230.86, 31.52%, 63.92%))",
  midPurple:"linear-gradient(hsl(275.11, 57.32%, 67.84%),hsl(275.11, 57.32%, 67.84%))",
...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className }) => {
  ...
  return (
    <div
      // className={`grid grid-cols-2 md:grid-cols-3 mx-auto py-[3em] overflow-visible ${
      //   className || ""
      // }`}
      className={`flex justify-center gap-4 sm:gap-8 md:gap-15 ${
        className || ""
      }`}
    >
      {items.map((item, index) => (
        <div key={index} className="flex justify-center">
          <button
            ...
            className={`relative bg-transparent outline-none w-[3.2em] h-[3.2em] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] group ${
              item.customClass || ""
            }`}
          >
            <a href={item.link} className="block w-full h-full">
              ...
              >
                <span
                  className="m-auto w-[2em] h-[2em] flex items-center justify-center"
                  aria-hidden="true"
                >
                  {item.icon}
                </span>
